...
/**
 * Auto Patch Transform Task for AGP 8.x
 */
abstract class AutoPatchTransformTask extends DefaultTask {
    ...
    void taskAction() {
        ...
        logger.quiet '================method signature to methodid is printed below================'
        JavaUtils.printMap(Config.methodMap)
        cost = (System.currentTimeMillis() - startTime) / 1000
        logger.quiet "autoPatch cost $cost second"

        // Write empty jar as output since patch is generated separately
    ...
    def initConfig() {
        ...
        ROBUST_DIR = "${projectDir}${File.separator}robust${File.separator}"
        ...
        Config.robustGenerateDirectory = "${buildDir}" + File.separator + "$Constants.ROBUST_GENERATE_DIRECTORY" + File.separator
        ...
        ReadXML.readXMl(projectDir.path)
        Config.methodMap = JavaUtils.getMapFromZippedFile(projectDir.path + Constants.METHOD_MAP_PATH)
    }

    List<CtClass> toCtClasses() {
        ...
        allDirectories.get().each { directory ->
            ...
            Config.classPool.insertClassPath(dirPath)
            org.apache.commons.io.FileUtils.listFiles(directory.asFile, null, true).each { file ->
                if (file.absolutePath.endsWith(".class")) {
                    def className = file.absolutePath.substring(dirPath.length() + 1, file.absolutePath.length() - ".class".length()).replaceAll(Matcher.quoteReplacement(File.separator), '.')
        ...
        allJars.get().each { jarInput ->
            Config.classPool.insertClassPath(jarInput.asFile.absolutePath)
        ...
        classNames.each {
            try {
                allClass.add(Config.classPool.get(it))
    ...
    def writeEmptyJar(File jarFile) {
        // Copy all input classes to output jar
        ...
        allDirectories.get().each { directory ->
            ...
            org.apache.commons.io.FileUtils.listFiles(directory.asFile, null, true).each { file ->
                if (file.absolutePath.endsWith(".class")) {
                    def entryName = file.absolutePath.substring(dirPath.length() + 1).replace(File.separator, '/')
                    ZipEntry entry = new ZipEntry(entryName)
                    zos.putNextEntry(entry)
        ...
        allJars.get().each { jarInput ->
            def jarFile2 = new JarFile(jarInput.asFile)
            Enumeration<JarEntry> entries = jarFile2.entries()
            while (entries.hasMoreElements()) {
                JarEntry jarEntry = entries.nextElement()
                if (!jarEntry.isDirectory()) {
                    ZipEntry entry = new ZipEntry(jarEntry.name)
                    zos.putNextEntry(entry)
                    InputStream is = jarFile2.getInputStream(jarEntry)
                    byte[] buffer = new byte[4096]
                    int len
